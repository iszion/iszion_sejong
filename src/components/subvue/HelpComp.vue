    ...
    <q-card class="my-card q-dialog-plugin">
      <q-card-section>
        <div class="row">
          ...
          <q-space />
          <q-toggle
            v-model="searchValue.searchAll"
            :label="`${searchValue.searchAll === 'Y' ? '사용중' : '출고정지'}`"
            color="pink"
            false-value="N"
            true-value="Y"
            @click="getData"
...
const columnDefs = ref([
  { headerName: '코드', field: 'compCd', maxWidth: 90, minWidth: 90, pinned: 'left' },
  { headerName: '상호(가칭명)', field: 'compNm', minWidth: 150 },
  { headerName: '상호(법인명)', field: 'compBusinNm', minWidth: 150 },
  { headerName: '사업자등록번호', field: 'compBusinNo', minWidth: 140, valueFormatter: businNoFormatter },
  { headerName: '대표자', field: 'compOwner', minWidth: 100 },
  { headerName: '출고정지', field: 'outDay', minWidth: 130, valueFormatter: dateFormatter },
]);

const rowSelection = ref(null);
const rowData = reactive({ rows: [] });
const searchValue = reactive({
  textValue: params.paramValue,
  outDay: params.paramCloseDay,
  searchAll: 'Y',
});
    ...
    if (rowData.rows.length === 1) {
      ...
      if (rowData.rows.length > 0) {
        setTimeout(() => {
          ...
          // 첫 번째 셀에 포커스 설정
          myGrid.value.api.setFocusedCell(focusedRowIndex, 'compNm'); // colId는 포커스를 줄 열의 ID
        }, 500);
      } else {
        setTimeout(() => {
          searchNmFocus().focus();
...
const getData = async () => {
  try {
    const response = await api.post('/api/com/helpComp_list', {
      paramValue: searchValue.textValue,
      paramOutDay: params.paramCloseDay,
      paramAll: searchValue.searchAll,
    });
