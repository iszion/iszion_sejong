    ...
    <q-dialog persistent full-height full-width v-model="isDialogVisible">
      <q-card class="q-pa-none q-ma-none">
        <q-card-section class="q-pa-none q-ma-none">
          <fee-v3020p :messages="{ rowData: rowData, titleNm: menuLabel, searchValue: searchValue }" @close="handleClose" />
        </q-card-section>
      </q-card>
    </q-dialog>
  </q-page>
...
import FeeV3020p from 'pages/erp/fee/FeeV3020p.vue';
...
const getData = async () => {
  try {
    const response = await api.post('/api/fee/fee3020_list', {
...
const calculateTotal = () => {
  ...
  const hasMeaningfulData = totalRow.sAmt !== 0 || totalRow.oAmt !== 0 || totalRow.oQty !== 0;
