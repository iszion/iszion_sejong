...
const getData = async () => {
  try {
    const response = await api.post('/api/sal/sal2160_list', {
...
const getDataSelect = async resData => {
  try {
    const response = await api.post('/api/sal/sal2160_select_list', {
      paramPeriodFrom: commUtil.unFormatDate(searchValue.period.from),
      paramPeriodTo: commUtil.unFormatDate(searchValue.period.to),
      paramProdCd: searchValue.prodCd,
...
const myGridOptions = {
  ...
  onRowClicked: function (event) {
    ...
    selectedNm.value = event.data.custNm;
...
const excelExport = (data, options) => {
  ...
  XLSX.utils.book_append_sheet(wb, ws, '거래처별판매현황');
  XLSX.writeFile(wb, '거래처별판매현황.xlsx');
};
...
const excelExportSub = (data, options) => {
  ...
  let wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, '거래처별도서판매현황');
  XLSX.writeFile(wb, '거래처별도서판매현황.xlsx');
};
