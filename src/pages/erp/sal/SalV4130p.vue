            ...
            <div id="printZone">
              ...
              <table>
                <tr>
                  ...
                  <th rowspan="2" colspan="1">이월</th>
                  <th rowspan="2" colspan="1">입고</th>
                  <th rowspan="1" colspan="4">출고현황</th>
                  <th rowspan="2" colspan="1">조정</th>
                  <th rowspan="2" colspan="1">재고</th>
                  <th rowspan="2" colspan="1">순출고계</th>
                ...
                <tr v-for="(data, index) in props.messages.rowData.rows || []" :key="index">
                  ...
                  <td>{{ commUtil.formatComma(data.xQty) }}</td>
                  <td>{{ commUtil.formatComma(data.zQty) }}</td>
                  <td>{{ commUtil.formatComma(data.yjQty) }}</td>
                  <td>{{ commUtil.formatComma(data.tQty) }}</td>
                  <td></td>
                </tr>
                <tr class="bg-grey3">
                  <td></td>
                  ...
                  <td>{{ commUtil.formatComma(props.messages.rowData.rowsSum[0].xQty) }}</td>
                  <td>{{ commUtil.formatComma(props.messages.rowData.rowsSum[0].zQty) }}</td>
                  <td>{{ commUtil.formatComma(props.messages.rowData.rowsSum[0].yjQty) }}</td>
                  <td>{{ commUtil.formatComma(props.messages.rowData.rowsSum[0].tQty) }}</td>
                  <td></td>
                </tr>
              </table>
              <div class="row">
...
const isPrintReport = () => {
  printJS({
    printable: 'printZone',
    type: 'html',
    css: ['/css/print/sal4130.css', '/css/quasar.css'],
    scanStyles: false,
  });
};
...
const headerGroup = reactive({
  header: [],
  headProps: ['prodNm', 'sPrice', 'wjQty', 'iQty', 'oQty', 'jQty', 'bQty', 'xQty', 'zQty', 'yjQty', 'jQty', 'remarks'],
  headRow1: [
    { name: '도서명', rowspan: 2, colspan: 1, key: 'prodNm' },
    { name: '정가', rowspan: 2, colspan: 1, key: 'sPrice' },
    { name: '이월', rowspan: 2, colspan: 1, key: 'wjQty' },
    { name: '입고', rowspan: 2, colspan: 1, key: 'iQty' },
    { name: '출고현황', rowspan: 1, colspan: 4 },
    { name: '조정', rowspan: 2, colspan: 1, key: 'zQty' },
    { name: '재고', rowspan: 2, colspan: 1, key: 'yjQty' },
    { name: '순출고계', rowspan: 2, colspan: 1, key: 'tQty' },
    { name: '기타사항', rowspan: 2, colspan: 1, key: 'remarks' },
  ],
  headRow2: [
    { name: '출고', rowspan: 1, colspan: 1, key: 'oQty' },
    { name: '증정', rowspan: 1, colspan: 1, key: 'jQty' },
    { name: '반품', rowspan: 1, colspan: 1, key: 'bQty' },
    { name: '폐기', rowspan: 1, colspan: 1, key: 'xQty' },
  ],
});
...
<style scoped>
@import 'src/css/print/sal4130.css';
</style>
