...
const isPrintReport = () => {
  printJS({
    printable: 'printZone',
    type: 'html',
    css: ['/css/print/sal1020.css', '/css/quasar.css'],
    scanStyles: false,
  });
};
...
const headerGroup = reactive({
  header: [],
  headProps: ['buyDay', 'seq', 'cnt', 'custNm', 'custCd', 'buyFgNm', 'prodNm', 'prodCd', 'qty', 'price', 'amt', 'buyYul', 'remarks'],
  headRow1: [
    { name: '입고일', rowspan: 1, colspan: 1, key: 'buyDay' },
    { name: '전표번호', rowspan: 1, colspan: 1, key: 'seq' },
    { name: 'seq', rowspan: 1, colspan: 1, key: 'cnt' },
    { name: '입고처', rowspan: 1, colspan: 1, key: 'custNm' },
    { name: '코드', rowspan: 1, colspan: 1, key: 'custCd' },
    { name: '구분', rowspan: 1, colspan: 1, key: 'buyFgNm' },
    ...
    { name: '비율', rowspan: 1, colspan: 1, key: 'buyYul' },
    { name: '기타사항', rowspan: 1, colspan: 1, key: 'remark' },
  ],
  // headRow2: [
...
const excelExport = (data, options) => {
  ...
  XLSX.utils.book_append_sheet(wb, ws, '매입자료현황');
  XLSX.writeFile(wb, '매입자료현황.xlsx');
};
</script>

<style scoped>
@import 'src/css/print/sal1020.css';
</style>
