...
const getDataProg = async () => {
  try {
    const response = await api.post('/api/com/com5010_menu_list', { groupCd: selectedGroup.value });
...
const getDataUser = async () => {
  try {
    const response = await api.post('/api/com/com2030_user_list', {
      paramCompCd: searchValue.compCd,
      paramDeptCd: searchValue.deptCd,
      paramProgId: selectedProg.value.progId,
    });
...
const saveDataUserAndHandleResult = resFormData => {
  api
    .post('/api/com/com2010_grntp_save', resFormData)
    .then(res => {
      showSaveUserBtn.value = false;
      let saveStatus = {};
...
const getDataGroup = async () => {
  try {
    const response = await api.post('/api/com/prog_group_list', {});
    // 옵션 초기화
...
async function getDataCommOption(resCommCd1) {
  try {
    ...
    switch (resCommCd1) {
      case '501':
        searchValue.deptOptions = response.data.data;
        searchValue.deptOptions.unshift({ commCd: '', commNm: '전체' });
        break;
      default:
